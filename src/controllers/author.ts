...
export const registerAuthor: RequestAuthorHandler  = async (req, res) =>{
    ...
    if(!user.signedUp){
        return sendErrorResponse({
             message: 'You must be signed up before you can register as author',
             status: 401,
            res
            })
    }
...
export const getAuthorDetails: RequestHandler = async (req, res) =>{
  ...
  res.json({id: author._id, name: author.name, about: author.about, socialLinks: author.socialLinks})
}